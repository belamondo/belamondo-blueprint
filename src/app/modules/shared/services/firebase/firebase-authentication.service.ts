...
import { fbAuth } from './../../../../../environments/environment';
...
export class FirebaseAuthenticationService {
  ...
  /*recoverPasswordEmail = (email) => new Promise((resolve, reject) =>  fbAuth.fetchProvidersForEmail(email)
    .then(res => {
      if(res.length > 0) {
  ...
  setUser = () => new Promise((resolve, reject) => {
    if(fbAuth.currentUser.emailVerified) {
      this.uid = fbAuth.currentUser.uid;
      resolve(this.uid);
    }
  })
